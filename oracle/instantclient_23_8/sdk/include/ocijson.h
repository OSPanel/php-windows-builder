...
/* 
   ...
   acolunga    10/25/24 - Bug 37202239: add OCIJsonBinaryHash
 ...
 *
 * PARAMETERS:
 *   svchp     (IN)  - OCI server context
 *   jsond     (IN)  - loaded JSON descriptor
 *   outbufp   (OUT) - buffer to write hash
 *   outbuf_sz (OUT) - size of written hash
 *   flags     (IN)  - control flags
 *   errhp     (IN)  - OCI error handler
 *   mode      (IN)  - OCI mode
 *
 * RETURNS:
 *   status indicating operation was successful or OCI_ERROR
 *
 * DESCRIPTION:
 *   This function receives a json descriptor and produces a
 *   JSON hash for it.
 */
sword OCIJsonBinaryHash (
  OCISvcCtx     *svchp,
  OCIJson       *jsond,
  ub1           *outbufp,
  ub4           *outbuf_sz,
